# -*- coding: utf-8 -*-
"""
Created on Fri Aug 10 15:33:58 2018
...
def reformat(dataset, labels):
  dataset = dataset.reshape((-1, image_size * image_size)).astype(np.float32) # 将每幅图由28x28的矩阵展开为1x784的向量
  # Map 0 to [1.0, 0.0, 0.0 ...], 1 to [0.0, 1.0, 0.0 ...]
  labels = (np.arange(num_labels) == labels[:,None]).astype(np.float32) #labels[:,None]将 labels的shape由(n,)变成(n,1)
...
with graph.as_default():
  ...
  tf_train_dataset = tf.constant(train_dataset[:train_subset, :]) # size: (10000,28^2)
  ...
  weights = tf.Variable( # size: (28^2, 10)
    tf.truncated_normal([image_size * image_size, num_labels])) # 从截断的正态分布中输出随机值。x的取值在区间（μ-2σ，μ+2σ）之外则重新进行选择
  ...
  logits = tf.matmul(tf_train_dataset, weights) + biases # Y = XW + b, size: (10000,10), 每个行向量对应一个sample的score，下一步进行softmax后对应于各个labels的概率
  loss = tf.reduce_mean( # 未指定第二个变量reduction_indices，因此在tensor的所有元素上求平均值
    tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf_train_labels, logits=logits)) #求所有样本的平均交叉熵
...
# The graph will be similar, except that instead of holding all the training data 
# into a constant node, we create a Placeholder node which will be fed actual data 
# at every call of session.run().
